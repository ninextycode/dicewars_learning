...
def extract_output_target(batch, model_output, mean_val, std_val):
    ...
    output_tr = torch.stack(output)
    target_tr = torch.stack(target)
    mean_val = mean_val.to(target_tr.device)
    std_val = std_val.to(target_tr.device)
    target_tr = (target_tr - mean_val) / std_val
    return output_tr, target_tr
